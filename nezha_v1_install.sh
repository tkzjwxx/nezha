#!/bin/bash
# ====================================================================
# 哪吒监控 V1 (Standalone) 极简全自动部署脚本 - Woiden/Hax 专供版 V3
...
clear
echo -e "${CYAN}=================================================================${NC}"
echo -e "${GREEN}      🚀 哪吒监控 V1 (独立版) 极简全自动部署引擎启动！ V3${NC}"
echo -e "${CYAN}=================================================================${NC}\n"

# --------------------------------------------------------
# 🚧 第一关：环境强提醒与域名录入 (加入输入除杂防抖)
# --------------------------------------------------------
...
while true; do
    ...
    # 强制清理输入内容中的空格和回车符，防止 Caddy 配置崩溃
...
# 🚀 第二关：强制清理历史残留与依赖更新
# --------------------------------------------------------
echo -e "${CYAN}📦 正在更新系统依赖并强制清理历史残留环境...${NC}"
apt update && apt install wget curl unzip jq -y >/dev/null 2>&1
systemctl stop docker nezha-dashboard nezha-agent caddy 2>/dev/null
apt purge docker.io containerd runc -y 2>/dev/null
# 强制拔除旧文件，防止残留的 config.yaml 干扰判断
...
# 绝对锁定法：寻找解压出来的 dashboard 核心文件并统一重命名
...
if ! systemctl is-active --quiet caddy; then
    echo -e "${RED}❌ Caddy 反代引擎启动失败！请检查域名解析或 CF 设置。${NC}"
...
while true; do
    read -p "👉 请在此处粘贴你刚刚复制的 Agent 密钥并回车: " RAW_SECRET
